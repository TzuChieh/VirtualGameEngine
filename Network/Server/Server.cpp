...
Server::Server(int port) {
	initialize();
	addr.sin_addr.s_addr = inet_addr(INADDR_ANY); //with portabilty
...
//			cout << "IP addr :" << client_addr.sin_addr.s_addr << endl;
			...
			this->totalUsers++;
//			CreateThread(0,0,(LPTHREAD_START_ROUTINE)clientHandlerThread,(LPVOID)(i),0,0);
...
bool Server::processPacket(int id, Packet packetType) {
	switch(packetType) {
		...
		default:
			cout << "unrecongnized packet: " << packetType << endl;
			break;
	} 
	return true;
}
